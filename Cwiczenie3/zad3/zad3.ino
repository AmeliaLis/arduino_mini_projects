...
// zakres 0-423V
	...
	float current = 0.0;
	float voltReader2 = analogRead(A15);
	float voltage2 = (voltReader2 / 1023.0) * 423;
	current = voltage2 / 211.5;
	
	displayVoltage(voltage, current);

}

void displayVoltage(float voltage, float current) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Voltage: ");
  lcd.print(voltage);
